window.addEventListener("load", () => {
  ...
  let Channel = params.filter((param) => {
    if (param.indexOf("Channel=") != -1) {
      return param;
    }
  });
  if ((Channel, length === 1)) {
    config.Channel = Channel[0].substring("Channel=".length);
  } else {
      ...
      let HandleUserJoin = async (user, MediaType) => {
        ...
        if (MediaType === "video") {
          document.querySelectorAll(".video-containers").forEach((element) => {
            arraysOfuid.push(
              Number(element.id.substring(14, element.id.length))
            );
          });
